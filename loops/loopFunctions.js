...
async function getMetaTempletByName(name, metaKeys, retries = 3, delay = 1000) {
  ...
  const url = `https://graph.facebook.com/v18.0/${metaKeys?.waba_id}/message_templates?name=${encodeURIComponent(requestedName)}`;
  const options = {
    method: "GET",
    headers: {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (Array.isArray(data?.data)) {
        const exactTemplate = data.data.find(
          (template) => template?.name === requestedName
        );

        return {
          ...data,
          data: exactTemplate ? [exactTemplate] : [],
          requestedTemplateName: requestedName,
          matchedTemplateName: exactTemplate?.name || null,
        };
      }
